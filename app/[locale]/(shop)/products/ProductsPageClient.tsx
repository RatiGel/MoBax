...
function ProductsPageInner({ products, categories, brands: brandRegistry, brandProducts }: ProductsPageClientProps) {
  ...
  useEffect(() => {
    ...
    setBrandLanding(brand && brandRegistry.find((b) => b.slug === brand) ? brand : null);
    setSelectedCategory((category as CategorySlug) || 'all');
    setSelectedBrand('all');
  }, [searchParams, brandRegistry]);
  ...
  const filtered = useMemo(() => {
    ...
    let result = brandLanding
      ? products.filter((p) => brandProducts[brandLanding]?.includes(p.slug))
      : [...products];
    ...
    return result;
  }, [products, categories, brandProducts, brandLanding, selectedCategory, selectedBrand, sortBy]);
          ...
          <div className="mb-10">
            ...
            <div className="space-y-1">
              ...
              {parentCategories.map((parent) => {
                const subs = categories.filter((c) => c.parentSlug === parent.slug);
